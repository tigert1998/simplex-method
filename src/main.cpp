...
private:
	...
	bool Adjust() {
		...
		for (u32 i = 0; i < m; i++) if (coe[i][n + m] < 0) need_adjust = true;
		if (!need_adjust) return true;
		...
		u32 min_b_id = m;
		{
			...
			for (u32 i = 0; i < m; i++) {
				a[i].resize(n + 1);
				a[i][0] = -1;
				for (u32 j = 0; j < n; j++) a[i][j + 1] = -coe[i][j];
				b[i] = coe[i][n + m];
		...
		for (u32 col_id : need_swap_in) {
			u32 row_id = m;
			{
				for (u32 i = 0; i < m; i++) if (coe[i][col_id] != 0 && !aux_solver.is_b[id[i] + 1]) {
					row_id = i;
					break;
				}
			}
	...
	void Log() const {
		...
		printf("B = { ");
		for (u32 i = 0; i < n + m; i++) if (is_b[i]) printf("%d, ", i);
		printf("}\n");
	}

	void Pivot(u32 row_id, u32 col_id) {
		u32 old_id_row_id = id[row_id];
		double divider = -coe[row_id][col_id];

		id[row_id] = col_id;
		coe[row_id][col_id] = 0;
		coe[row_id][old_id_row_id] = -1;
		for (u32 i = 0; i <= n + m; i++) coe[row_id][i] /= divider;
...
public:
	enum class ResultType {
		kUnbounded, kSolved, kInfeasible
	};

	void Reset(const vector<vector<double>> &a, const vector<double> &b, const vector<double> &c) {
		m = a.size();
		if (m == 0) throw invalid_argument("a");
		n = a.front().size();
		if (n == 0) throw invalid_argument("a");
		for (u32 i = 1; i < m; i++) if (a[i].size() != n) throw invalid_argument("a");
		if (b.size() != m) throw invalid_argument("b");
		if (c.size() != n) throw invalid_argument("c");
		...
		for (u32 i = 0; i < m; i++) {
			...
			for (u32 j = 0; j < n; j++) coe[i][j] = -a[i][j];
			coe[i][n + m] = b[i];
		}
	...
	tuple<ResultType, double> Solve() {
		using ReturnType = tuple<ResultType, double>;
		...
		while (true) {
			...
			u32 row_id = m;
			{
				double min_delta;
				for (u32 i = 0; i < m; i++) {
					if (coe[i][col_id] >= 0) continue;
					double tmp = coe[i][n + m] / (-coe[i][col_id]);
